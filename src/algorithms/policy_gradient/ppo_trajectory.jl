...
function PPOTrajectory(;capacity, action_log_prob_size=(), action_log_prob_type=Float32, kw...)
    ...
    PPOTrajectory{
        typeof(t),
        typeof(p),
        (:state, :action, :action_log_prob, :reward, :terminal, :next_state, :next_action, :next_action_log_prob),
    Tuple{types.parameters[1:2]..., frame_type(p), types.parameters[3:end]..., frame_type(p)},
    }(
        t,
        p,
    )
...
Base.getindex(t::PPOTrajectory, s::Symbol) = s == :action_log_prob ? t.action_log_prob : t.trajectory[s]
