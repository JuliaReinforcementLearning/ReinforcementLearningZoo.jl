...
"""
Base.@kwdef struct DoubleLearner{T1<:AbstractLearner,T2<:AbstractLearner,R<:AbstractRNG} <: AbstractLearner
    L1::T1
    L2::T2
...
function RLBase.update!(L::DoubleLearner{<:TDLearner}, t::AbstractTrajectory, ::AbstractEnv, s::Union{PreActStage, PostEpisodeStage})
    if rand(L.rng, Bool)
        L, Lₜ = L.L1, L.L2
    else
        L, Lₜ = L.L2, L.L1
    ...
    if s === POST_EPISODE_STAGE
        ...
        if length(R) >= n+1
            ...
            G = discount_rewards_reduced(@view(R[end-n:end]), γ) + γ^(n+1) * Qₜ(s′, argmax(Q(s′)))
            update!(Q, (s,a) => Q(s ,a) - G)
        end
    end
end
