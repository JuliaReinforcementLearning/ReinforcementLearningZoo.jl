...
function RLCore.Experiment(
    ...
    function create_model()
        layers = Chain(
            ...
            CrossCor((3, 3), 64 => 64, relu; stride = 1, pad = 1, init = init),
            x -> reshape(x, :, size(x)[end]),
            Dense(11 * 11 * 64, 1024, relu; initW = init),
            Dense(1024, 512, relu; initW = init),
        )
        V = Dense(512, 1, initW = x),
        A = Dense(512, N_ACTIONS, initW = x)
        return V, A
    end
    ...
    agent = Agent(
        policy = QBasedPolicy(
            learner = DQNLearner(
                ...
                target_approximator = NeuralNetworkApproximator(model = create_model() |> gpu),
                update_freq = 4,
                γ = 0.99f0,
                update_horizon = 1,
                batch_size = 32,
